...
def run_test_integers():
    ...
    # Test 2:
    expected = [3, 10]
    answer = integers([['omae wa mo shindeiru', 3, 10, 13.66], ['nani?']])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
...
def integers(sequence_of_sequences):
    ...
    for a in range(len(sequence_of_sequences)):
        for b in range(len(sequence_of_sequences[a])):
            if type(sequence_of_sequences[a]) is list:
                for b in range(len(sequence_of_sequences[a])):
                    if type(sequence_of_sequences[a][b]) is int:
                        x_list.append(sequence_of_sequences[a][b])
            elif type(sequence_of_sequences[a]) is int:
                x_list.append((sequence_of_sequences[a]))
    return x_list
...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'OMAESDNA'
    answer = big_letters(["OMAE wa, mo ShinDeiru",
                         "NanI?"])
...
def big_letters(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        for j in range(len(sequence_of_sequences[k])):
            if sequence_of_sequences[k][j] is str:
                for z in range(len(sequence_of_sequences[k][j])):
                    if sequence_of_sequences[k][j][z].isupper():
                        x_str += (sequence_of_sequences[k][j][z])
